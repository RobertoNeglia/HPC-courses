...
/* To write Matrices or Vectors in MatrixMarket format use
   ...
   template<typename SparseMatrixType>
   bool saveMarket(const SparseMatrixType& mat, const std::string& filename, int
   sym = 0)
   template<typename VectorType>
...
read_MM_Matrix(const std::string &filename)
{
  ...
  std::regex    rsym(" symmetric", std::regex_constants::ECMAScript |
              ...
              if(M != N)
                {
                  throw std::runtime_error(
                    std::string("Error while reding matrix. A symmetric matrix "
                                "must be square"));
                }
              nonZeros = 2 * NNZ - M;
            }
...
template <typename SparseMatrixType>
bool
symLoadMarket(SparseMatrixType &mat, const std::string &filename)
{
  typedef typename SparseMatrixType::Scalar Scalar;
          ...
          if(M > 0 && N > 0 && NNZ > 0)
            {
              if(N != M)
                throw std::runtime_error(
                  "Symmetric matricies should be square");
              readsizes = true;
              mat.resize(M, N);
              mat.reserve(2 * NNZ - N);
            }
...
read_MM_Matrix(const std::string &filename)
{
  ...
  if(symmetric)
    ...
    std::cout << "Unymmetric matrix" << std::endl;
  if(iscomplex)
  ...
  if(isvector)
    {
      throw std::runtime_error(
        std::string("read_MM_Matrix cannot read a vector"));
    }
  if(iscomplex)
    {
      if(!std::is_same<std::complex<double>, Scalar>::value)
        {
          throw std::runtime_error(
            std::string("read_MM_Matrix: wrong matrix type: read_MM_Matrix is "
                        "only for matrices of reals"));
        }
    }
  ...
  if(!good)
    throw std::runtime_error(std::string("read_MM_Matrix: CANNOT READ MATRIX"));
  return mat;
}
