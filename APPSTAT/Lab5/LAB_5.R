...
# p-value is 0.0794, so we don't reject H0 at level 5% (but we reject at 10%)
...
D.T2 < cfr.fisher
# FALSE: we reject H0 at level 5%

# we compute the p-value
P <- 1 - pf(D.T2 * (n - p) / (p * (n - 1)), p, n - p)
P
# reject H0 at 5% (don't reject at 1%)
# I'm rejecting the hypothesis that the two measurements have the same mean
# -> altough we are measuring the same thing, the two laboratories
#     don't agree on the results.
# The p-value says that if H0 is true, the probability of observing
# what I have observed is 2%
...
# Adding delta.0 (H0) and the quadrants
points(
  delta.0[1],
  delta.0[2],
  pch = 16,
...
# So I reject the hypothesis that the two measurements have the same mean,
# but I don't know which one is different from the other.

# Now, let's communicate our results to the client
# (because we need to take our salary!)
...
# If we were only to look at the intervals, we wouldn't reject H0
# because both the intervals contains delta.0
...
worst
# direction along which the maximum is realized

# Confidence interval along the worst direction:
IC.worst <-
  c(
    # coefficients along the worst direction
    D.mean %*% worst - sqrt(cfr.fisher * (t(worst) %*% D.cov %*% worst) / n),
    D.mean %*% worst,
    D.mean %*% worst + sqrt(cfr.fisher * (t(worst) %*% D.cov %*% worst) / n)
  )
IC.worst
# it's not containing the projection of our mean along the worst direction
delta.0 %*% worst
(IC.worst[1] < delta.0 %*% worst) &
  (delta.0 %*% worst < IC.worst[3])
# Reject H0: a'mu == a'delta.0 in direction a = worst

# Extremes of IC.worst in the coordinate system (x,y):
# this is coeff * direction
x.min <-
  IC.worst[1] * worst # (x,y) coords of the lower bound of the interval
x.min
x.max <-
  IC.worst[3] * worst # (x,y) coords of the upper bound of the interval
x.max
m1.ort <-
  -worst[1] / worst[2] # Slope of the line orthogonal to worst
q.min.ort <-
  x.min[2] - m1.ort * x.min[1] # Intercept of line of slope m1.ort
# passing by x.min
q.max.ort <-
  x.max[2] - m1.ort * x.max[1] # Intercept of line of slope m1.ort
# passing by x.max
abline(
  q.min.ort,
  m1.ort,
...
for (i in 1:length(theta))
{
  ...
  t2 <-
    (mean(D %*% a) - (delta.0 %*% a))^2 / (var(D %*% a) / n)
  T2.d <- c(T2.d, t2)
  centerf <- D.mean %*% a
  maxf <-
    D.mean %*% a + sqrt(t(a) %*% D.cov %*% a / n) * sqrt(cfr.fisher)
  minf <-
    D.mean %*% a - sqrt(t(a) %*% D.cov %*% a / n) * sqrt(cfr.fisher)
...
plot(
  theta,
  Centerf,
  main = "Simultaneous T2 confidence intervals",
  ylim = c(-30, 35),
  col = "grey25",
  type = "l",
  ylab = "IC"
...
lines(c(theta[1], theta[1]),
  c(Minf[1], Maxf[1]),
  col = "red",
  lwd = 2
)
lines(c(theta[91], theta[91]),
  c(Minf[91], Maxf[91]),
  col = "red",
  lwd = 2
)
...
lines(c(theta[which.max(T2.d)], theta[which.max(T2.d)]),
  c(Minf[which.max(T2.d)], Maxf[which.max(T2.d)]),
  col = "forestgreen",
  lwd = 2
)
...
for (i in 1:length(theta)) {
  ...
  lines(
    rbind(
      D.mean + as.vector(sqrt(var(as.matrix(
        D
      ) %*% a) / n) * qt(1 - alpha / (2 * k), n - 1)) * a + 100 * a.orth,
      D.mean + as.vector(sqrt(var(as.matrix(
        D
      ) %*% a) / n) * qt(1 - alpha / (2 * k), n - 1)) * a - 100 * a.orth
    ),
...
for (i in 1:length(theta)) {
  ...
  lines(
    rbind(
      D.mean + as.vector(sqrt(var(as.matrix(
        D
      ) %*% a) / n) * sqrt(cfr.fisher)) * a + 100 * a.orth,
      D.mean + as.vector(sqrt(var(as.matrix(
        D
      ) %*% a) / n) * sqrt(cfr.fisher)) * a - 100 * a.orth
    ),
...
# Rational conclusion: the two labs are not doing the same thing
# By simply looking at the scatter plot, we can see that the first lab is
# overestimating SS, while underestimating BOD
# (or the viceversa, we don't know)
...
# Plotting each observation as a line with
# 'h.0', 'h.8', 'h.16', 'h.24' on x-axis
...
C
# First column of the contrast matrix represent a "baseline" observation,
# the other columns represent the differences between the observations

# Here we are looking at the effects on the pressure after 8, 16 and 24 hours
# from the instant the drug was given
...
Sd <-
  C %*% S %*% t(C) # Sample covariance of the constrasted observations
Sdinv <- solve(Sd)
...
cfr.fisher <- ((q - 1) * (n - 1) / (n - (q - 1))) * qf(1 - alpha, (q - 1), n -
  (q - 1))

T2 < cfr.fisher # Testing if we are in the rejection region
# indeed we are => we reject H0 -> the drug has an effect on the blood pressure
T2
cfr.fisher
# T2 is much higher than cfr.fisher => the p-value will be very small
P <- 1 - pf(T2 * (n - (q - 1)) / ((q - 1) * (n - 1)), (q - 1), n - (q -
  1))
P
...
IC.T2
dimnames(IC.T2) <- list(c("8h", "16h", "24h"), c("lower", "mean", "upper"))
...
T2bis < cfr.fisher
# again we are in the rejection region => we reject H0
...
# (this is clearly false, but we want to see how to test it)
...
# and in fact we reject H0

# _____________________________________________________________________________
##### Test for the mean of two independent Gaussian populations
##### ------------------------------------------------------------

# RMK: this is not like the paired situation, where we have a single sample
# of n pairs of observations, but we have two independent samples of resp.
# n1 and n2 observations

# In this case we compute the mean of the first sample, the mean on the second
# sample, and we make inference on the two means: at no moment we compute
# the difference between the two means, because there is no pairing between
# the two samples
...
# RMK: for ANOVA and MANOVA we need the two covariance matrix to be equal
...
T2 <-
  n1 * n2 / (n1 + n2) * (t1.mean - t2.mean - delta.0) %*% Spinv %*% (t1.mean -
    t2.mean - delta.0)

cfr.fisher <- (p * (n1 + n2 - 2) / (n1 + n2 - 1 - p)) * qf(1 - alpha, p, n1 +
  n2 - 1 - p)
T2 < cfr.fisher
# TRUE: no statistical evidence to reject H0 at level 1%

P <- 1 - pf(T2 / (p * (n1 + n2 - 2) / (n1 + n2 - 1 - p)), p, n1 + n2 - 1 -
  p)
P
...
IC.T2.X1 <-
  c(
    ...
    t1.mean[1] - t2.mean[1] + sqrt(cfr.fisher * Sp[1, 1] * (1 /
      n1 + 1 / n2))
  )
IC.T2.X2 <-
  c(
    t1.mean[2] - t2.mean[2] - sqrt(cfr.fisher * Sp[2, 2] * (1 / n1 + 1 / n2)),
    t1.mean[2] - t2.mean[2] + sqrt(cfr.fisher * Sp[2, 2] * (1 /
      n1 + 1 / n2))
  )
...
# the presence (1) or absence (0) of 520 mutations, for the two groups
# respectively.
# The geneticist suspects that some of these mutations might be positively
# associated with the allergy to almonds, and for this reason they decide to
# compare the incidence of the 520 mutations in the two populations.
...
#     -> Z-test on the mean of the two groups
# b) Report the significant mutations, imposing a probability of at most 1%
#    that at least one of the non-influential mutations is judged as
#    influential.
#     -> FWER (Bonferroni strategy)
# c) Report the significant mutations, imposing that the expected proportion
#    of mutations erroneously judged to be influential among all those judged
#    to be influential is lower than or equal to 1%
#     -> FDR  (Benjamini-Hochberg procedure)

# Type I error: we reject H0 when it is true
# Type II error: we don't reject H0 when it is false
...
n1 <- dim(allergy)[1] # n1 = 100
n2 <- dim(noallergy)[1] # n2 = 100
p <- dim(noallergy)[2] # p = 520

# Variables are binary -> Bernoulli distribution
...
# pooled estimate for the mean
p.hat <- (x.mean1 * n1 + x.mean2 * n2) / (n1 + n2)
p_hat # pooled estimate for the variance (Bernoulli distribution variance)
x.var <- (p.hat * (1 - p.hat))

# Basically I'm trying to find the mutations for which the mean of the two
# groups is significantly different, i.e., the mutations that affect the
# probability of being allergic to almonds.
...
# Asymptotic Z-test for the comparison of proportions
# (univariate and n=100 -> OK)
# https://vitalflux.com/two-samples-z-test-for-means-formula-examples/

# Assume mu.i1 = mu.i2 -> mu.i1 - mu.i2 = 0
# first compute the test statistics
z.i <- (x.mean1 - x.mean2) / sqrt(x.var * (1 / n1 + 1 / n2))
# then compute the p-values
p.i <- ifelse(z.i < 0, 2 * pnorm(z.i), 2 * (1 - pnorm(z.i)))

# find the significant mutations
which(p.i < .01)

# b) Report the significant mutations, imposing a probability of at most 1%
#    that at least one of the non-influential mutations is judged as
#    influential.
#     -> FWER (Bonferroni strategy) -> very conservative
# Bonferroni test
k <- 520
...
# as we can see the Bonferroni method is VERY conservative.
