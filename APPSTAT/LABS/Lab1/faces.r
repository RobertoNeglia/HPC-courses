faces <- function(xy = rbind(1:3, 5:3, 3:5, 5:7),
                  which.row,
                  fill = F,
                  nrow,
                  ncol,
  ...
  if (byrow)
    xy <- t(xy)
  if (!missing(which.row) &&
      all(!is.na(match(which.row, 1:dim(xy)[2]))))
    xy <- xy[, which.row, drop = F]
  mm <- dim(xy)[2]
  n <- dim(xy)[1]
  xnames <- dimnames(xy)[[1]]
  if (is.null(xnames))
    xnames <- as.character(1:n)
  if (!missing(labels))
    xnames <- labels
  if (scale) {
  ...
  for (ind in 1:n) {
    ...
    m <- 0
    face$hair[c("hair1", "hair2"), 2] <-
      face$hair[c("hair1", "hair2"), 2] + 50 * factors[11]
    
    m <- mean(face$nose[, 2])
    face$nose[, 2] <- m + (face$nose[, 2] - m) * (1 + 0.7 * factors[12])
    face$nose[nose.xnotnull, 1] <-
      face$nose[nose.xnotnull, 1] * (1 + factors[13])
    ...
    face.obj <- list(
      ...
      hair = rbind(
        face$shape["hairend", ],
        face$hair,
        invert(face$hair[hair.refl.ind, ]),
