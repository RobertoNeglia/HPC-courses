...
summary(fm)
# T-statistic: beta_i == 0 vs beta_i != 0: significance of the single
# parameter. RMK: ONE AT A TIME!
# tests if the i-th parameter is useful or not on predicting the target var.
# F-statistic: test the significance of all the parameters simultaneously:
# all parameters are useless vs at least one is useful
# Note: colinearity


# In this case we have:
#    - pvalue associated with the significance of the intercept is high
#         -> we don't reject, so we don't have evidence of the fact that
#            the intercept is not 0
#    - the same for speed1 and speed2
#    However, we can see that the pvalue associated with the F-statistic is
#    very low, so we can reject the null hypothesis that all the parameters
#    are useless, and hence we know we are explaining something with the model
# The high pvalues associated with the single parameters are due to the fact
# that by testing them one at a time we are not taking into account the
# colinearity between the parameters!
...
# Here I'm testing if C*beta == 0, where C = (0, 1, 0; 0, 0, 1)
...
abline(v = 0)
abline(h = 0)
# RMK: (0, 0) is outside the confidence region, so we exclude at 95% of
# confidence level the fact that both the parameters are 0 at the same time
#    -> but by looking at the single parameters along the axis separately
#       we can see that both are inside the projection of the ellipse on
#       the axis (so we would not reject the null hypothesis that they are 0)
# High colinearity -> Bonferroni is very different from the ellipse
# Note: colinearity!
# very very high correlation between the two parameters: this is due to the fact
# that in the range that we are considering the speed^2 is almost equal to the
# speed, so the two parameters are almost the same

# Bonferroni intervals (level 95%)
Bf <- rbind(
     beta1 = c(
          coefficients(fm)[2] - sqrt(vcov(fm)[2, 2]) *
               qt(1 - 0.05 / (2 * p), n - (r + 1)),
          coefficients(fm)[2] + sqrt(vcov(fm)[2, 2]) *
               qt(1 - 0.05 / (2 * p), n - (r + 1))
     ),
     beta2 = c(
          coefficients(fm)[3] - sqrt(vcov(fm)[3, 3]) *
               qt(1 - 0.05 / (2 * p), n - (r + 1)),
          coefficients(fm)[3] + sqrt(vcov(fm)[3, 3]) *
               qt(1 - 0.05 / (2 * p), n - (r + 1))
     )
...
abline(v = Bf[1, 1], col = "red")
abline(v = Bf[1, 2], col = "red")
abline(h = Bf[2, 1], col = "red")
abline(h = Bf[2, 2], col = "red")
...
Conf <-
     predict(fm, Z0.new, interval = "confidence", level = 1 - 0.05)
Conf
# Pred. int. for a new obs
Pred <-
     predict(fm, Z0.new, interval = "prediction", level = 1 - 0.05)
Pred
...
# I try removing outliers
res <- residuals(fm)
res.mean <- mean(res)
res.var <- var(res)
res.var

d.mahalanobis <- (res - res.mean)^2 / res.var
plot(d.mahalanobis)

cars.wo.outliers <- cars[d.mahalanobis < 2, ]
par(mfrow = c(1, 2))
plot(cars)
plot(cars.wo.outliers)

dist.woutliers <- cars.wo.outliers$dist
speed1.woutliers <- cars.wo.outliers$speed
speed2.woutliers <- cars.wo.outliers$speed^2

fm.woutliers <- lm(dist.woutliers ~ speed1.woutliers + speed2.woutliers)
summary(fm.woutliers)
...
shapiro.test(residuals(fm.woutliers))

residuals(fm.woutliers)

par(mfrow = c(2, 2))
plot(fm.woutliers)

res.wo.outliers <- res[d.mahalanobis < 3]

plot(res.wo.outliers)
qqnorm(res.wo.outliers)
qqline(res.wo.outliers)

shapiro.test(res.wo.outliers)

### RMK: points that are showing non normality in the qqplot are not
### necessarily outliers!!! -> I have to check the residuals that have
### a high leverage! -> they're the ones that are influencing the
### model the most, high-leverage points have no neighboring points in
### R^p space
...
##### --------------------------------------------------------
rm(list = ls())
setwd("~/HPC/APPSTAT/Lab9")

data <- read.table("brain_weight.txt", header = TRUE)
...
plot(
     X,
     Y,
     main = "Scatterplot brain weight vs body weight",
     lwd = 2,
     xlab = "Body weight",
     ylab = "Brain weight"
)
...
summary(result)
# R-squared: 0.8727
# I'm able to explain 87% of the variability of the brain weight with the body
# weight.
...
par(mfrow = c(2, 2))
plot(result)
# Look at Cook's distance: it's a measure of the influence of each point on the
# model. It's a measure of the distance between the fitted value and the actual
# value. The higher the Cook's distance, the higher the influence of the point
# on the model.
# These points also have a high leverage: they're the points that are far away
# from the center of the data.
# In this case, the points with the highest Cook's distance are the ones with
# the highest body weight. They're the points that are influencing the model
# the most.
...
# The fact is that they're not outliers, they're just different animals!
# so it doesn't make sense to exclude them.
# -> Transform the data!
...
plot(
     log.X,
     log.Y,
     main = "Scatterplot ln(Brain weight) vs ln(Body weight)",
     lwd = 2,
     xlab = "ln(Body weight)",
...
### RMK: data is now in log scale, so the coefficients are not summed as before
### but multiplied!
### instead of having Y = beta0 + beta1 * X, we have
### ln(Y) = beta0 + beta1 * ln(X), so Y = exp(beta0) * X^beta1
...
X.new.log <-
     data.frame(log.X = seq(min(log.X), max(log.X), len = 100))

IC.log <-
     predict(result.log, X.new.log, interval = "confidence", level = 0.95)
...
IP.log <-
     predict(result.log, X.new.log, interval = "prediction", level = 0.95)
...
# regression line
beta.0 <- exp(coef.log[1])
beta.1 <- coef.log[2]
x <- seq(min(X), max(X), len = 100)
lines(x, beta.0[1] * x^beta.1)
...
summary(fit)
# nice p-values, but very small R^2! -> less than 3% of the variability is
...
surface3d(range(Q$lat), range(Q$long),
     matrix(predict(
          fit, expand.grid(lat = range(Q$lat), long = range(Q$long))
     ), 2, 2),
     alpha = 0.5
)
...
### But I know that the data are clustered in two groups, so I can try to
### use this fact and introduce a dummy variable to model the two groups
...
# i.e.,
# depth = B0[g] + B1[g]*lat + B2[g]*long + eps with B0[g] such that:
#    For the intercept
#      B0[g]=beta0       if the unit is in group s.t. dummy=0 (red)
#      B0[g]=beta0+beta3 if the unit is in group s.t. dummy=1 (green)
#    For the latitude interactions
#      B1[g]=beta1       if the unit is in group s.t. dummy=0 (red)
#      B1[g]=beta1+beta4 if the unit is in group s.t. dummy=1 (green)
#    For the longitude interactions
#      B2[g]=beta2       if the unit is in group s.t. dummy=0 (red)
#      B2[g]=beta2+beta5 if the unit is in group s.t. dummy=1 (green)

## RMK: If I were tu add only the dummy variable in beta3, I would have
## a change in the intercept, but not in the slope. So I need to add
## the interaction terms to have a change in the slope too.

fitd <-
     lm(depth ~ lat + long + dummy + lat:dummy + long:dummy, data = Qd)
summary(fitd)
### RMK: lat:dummy doesn't seem to be significant
...
A <-
     rbind(c(0, 0, 0, 1, 0, 0), c(0, 0, 0, 0, 1, 0), c(0, 0, 0, 0, 0, 1))
b <- c(0, 0, 0)
linearHypothesis(fitd, A, b)
# Here I'm testing the null hypothesis stating that the coefficients beta3,
# beta4 and beta5 are all zero. The p-value is very small, so I can reject
# the null hypothesis and conclude that these three are indeed different
# from zero, and hence that the two planes are needed.
...
## now all the coefficients seems to be significant
...
# we still have some outliers (in the residuals), but overall they don't
# have a high leverage (influence), and neither a high Cook's distance
...
## first picture: high bias, low variance
## last picture: low bias, high variance
## perfect tradeoff: 3rd picture of the top row

fit.sin <- lm(y ~ sin(x))
summary(fit.sin)
plot(x, y, pch = 20)
points(x, y.new, col = "red")
data.plot.sin <- data.frame(x = x.plot, sin = sin(x.plot))
lines(x.plot, predict(fit.sin, data.plot.sin))
...
dGer <-
     rep(c(0, 1, 0), c(10, 10, 10)) # dGer= 1 if Germany, 0 otherwise
dFr
dGer
...
fit <-
     lm(Reg ~ dFr + dGer + Year2 + Year2:dFr + Year2:dGer, data = dati)
...
people <- read.table("people.txt", header = TRUE)
people
...
C <-
     rbind(
          c(1, (1 - cos(2 * pi / 12 * 30))), # total waste in June 2011 [mese=30]
          c(1, 0), # waste due to residents in June 2011
          c(0, (1 - cos(2 * pi / 12 * 30)))
     ) # waste due to tourists in June 2011
C
...
data <- read.table("work.txt", header = TRUE)
head(data)
