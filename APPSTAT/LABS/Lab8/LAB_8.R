...
install.packages(c("dbscan", "fields"))
...
# let's forget the labels (we perform a cluster analysis,
# not a discriminant one)
...
plot(iris4,
  col = ifelse(cluster.es == 1, "red", "blue"),
  pch = 19
)

plot(iris4,
  col = ifelse(cluster.ec == 1, "red", "blue"),
  pch = 19
)

plot(iris4,
  col = ifelse(cluster.ea == 1, "red", "blue"),
  pch = 19
)
...
# Cophenetic distance: at which distance did the two units i and j aggregate?
coph.es <- cophenetic(iris.es)
coph.ec <- cophenetic(iris.ec)
coph.ea <- cophenetic(iris.ea)

# I want cophenetic matrices to be as close as possible to the dissimilarity
# matrix used to perform the clustering
...
# I have higher correlation with the Euclidean distance matrix when I use
# the single linkage and average linkage methods, while I have a lower
# correlation with the complete linkage method
...
plot(
  ...
  xlab = "Var 1",
  ylab = "Var 2",
  main = "Euclidean, Average linkage",
  col = cluster.ea + 1,
  pch = 16,
  asp = 1
)
...
# 'loss of information' from joining two groups.
# This method is usually implemented with loss of information taken to be an
# increase in an error sum of squares criterion, ESS.
# First for a given cluster k, let ESS[k] be the sum of the squared deviations
# of every item in the cluster from the cluster mean (centroid).
...
# combination results in the smallest increase in ESS
# (minimum loss of information) are joined.
# Initially, each cluster consists of a single item and, if there are N items,
# ESS[k]=0, k=1,2,...,N, so ESS=0. At the other extreme, when all the clusters
# are combined in a single group of N items, the value of ESS is given by
# ESS=sum_j(t(x.j-x.mean)%*%(x.j-x.mean)), where x.j is the multivariate
# measurement associated with the jth item and x.mean is the mean of all items.
...
for (i in 1:iter.max) {
  ...
  QC <- rbind(C, x)
  Dist <-
    as.matrix(dist(QC, method = "euclidean"))[(k + 1):(k + n), 1:k]
  for (j in 1:n) {
...
result.k <-
  kmeans(Q, centers = 2) # Centers: fixed number of clusters
...
result.k$centers
# tot. sum of squares (total sum of squares when therea are no clusters)
result.k$totss
# sum of squares within clusters (sum of squares within each cluster)
result.k$withinss
# sum(sum of squares within cluster)
# this is the W(k) function (see lecture notes)
...
###   the variability within the groups (something like (M)ANOVA)
...
points(1:10, w / (w + b), type = "b", pch = 19, cex = 1)
...
x <-
  cbind(
    x = runif(4) + rnorm(n, sd = 0.1),
    y = runif(4) + rnorm(n, sd = 0.1)
  )
...
# Index of non noise points
clustered_index <-
  which(dbs$cluster != 0)
# only clustered points
clustered_points <- x[clustered_index]
# corresponding labels
clustered_labels <-
  dbs$cluster[clustered_index]
...
# silhouette score is almost -1, which is not very good: in this case we
# either have too many clusters or too few clusters (since we generated the
# data, we know that we have 4 clusters, so we have too many clusters)
...
dbscan_perf <- function(minPts, eps) {
  ...
  clustered_index <-
    which(dbs$cluster != 0) # Index of non noise points
  clustered_points <- x[clustered_index] # only clustered points
  clustered_labels <-
    dbs$cluster[clustered_index] # corresponding labels
  nb_clusters <- length(unique(clustered_labels))

  if ((nb_clusters > 1 &
    nb_clusters < n) &
    (length(which(dbs$cluster == 0)) / n < max_share_noise)) {
...
perf_grid <-
  outer(minPts_grid, eps_grid, FUN = Vectorize(dbscan_perf))
...
image.plot(
  ...
  col = hcl.colors(64, palette = "Blue-Red"),
...
hclust.w <- hclust(dist(moons), method = "ward.D2")
...
cluster.w <- cutree(hclust.w, k = 2) # euclidean-ward
...
vowels <- read.table("veritatis.txt", header = TRUE)
head(vowels)
dim(vowels)
...
Spinv <- solve(Sp)
T2 <-
  n1 * n2 / (n1 + n2) * (t1.mean - t2.mean - delta.0) %*% Spinv %*% (t1.mean - t2.mean - delta.0)
P <-
  1 - pf(T2 / (p * (n1 + n2 - 2) / (n1 + n2 - 1 - p)), p, n1 + n2 - 1 - p)
P
...
satellite <- read.table("satellite.txt", header = TRUE)
head(satellite)
...
alpha <- 0.01
cfr.fisher <-
  (p * (n1 + n2 - 2) / (n1 + n2 - 1 - p)) * qf(1 - alpha, p, n1 + n2 - 1 - p)
...
gruppi <-
  hclust(dist(sparrows, method = "manhattan"), method = "single")
plot(
  gruppi,
  hang = -0.1,
  sub = "",
...
T2 <-
  n1 * n2 / (n1 + n2) * (mean1 - mean2 - delta.0) %*% Spinv %*% (mean1 - mean2 - delta.0)
...
text(
  location[, 1],
  location[, 2],
  labels = colnames(as.matrix(eurodist)),
  cex = 0.75,
  pos = 3
)
...
for (k in 1:4)
{
  ...
  Stress <-
    (sum((as.vector(eurodist) - as.vector(dist(
      location.k
    )))^2) /
