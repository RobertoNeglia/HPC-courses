...
quartz()
plot(cyto[,1], cyto[,2], pch=19, col=c(rep('blue',8), rep('red',5)),
...
plot(x, PA * dnorm(x, MA, sqrt(S)), type='l', col='blue',
...
legend(-10, 0.03, legend=c(expression(paste('P(A)',f[A],'(x)')),
...
legend(-10, 0.9, legend=c('P(A|X=x)', 'P(B|X=x)'), col=c('blue','red'), lty=1,
       cex = 0.7)
