...
# p-value is greater than most common significance levels, so we can't reject
# the null hypothesis of normality, i.e., the data seem Gaussian.
...
# again, p-value is greater than most common significance levels, so we can't
# reject the null hypothesis of normality, i.e., the data seem Gaussian.

# Royston’s test (Shapiro-Wilk test under the hood)
result <- mvn(data = X, mvnTest = "royston")
result$multivariateNormality
# same result as the two previous tests
...
rm(list = ls())
setwd("~/HPC/APPSTAT/Lab4")

stiff <- read.table("stiff.dat")
head(stiff)

plot(stiff, asp = 1, pch = 19)
# positive correlation between all variables
...
result$multivariateNormality
# again, p-value is very small, so we reject the null hypothesis

### The data don't seem Gaussian. What can we do?
### 1 - Identify clusters
### 2 - Identify (and possibly remove) outliers
### 3 - Transform the data (e.g., Box-Cox transformations)
...
dim(stiff_wo_outliers)


result <-
   mvn(
      data = stiff_wo_outliers,
      multivariatePlot = "qq",
      mvn = "royston",
      covariance = FALSE
   )
...
# For lambda < 1 (black and yellow points in the plot): observations < 1
# are "spread", observations > 1 are "shrinked";
# For lambda > 1 (green points): observations < 1 are "shrinked",
# observations > 1 are "spread"
...
# we would need to spread the observations with small values and shrink the
...
bc.x <-
   bcPower(x, lambda.x$lambda)
# it transforms the data of the first argument through the Box-Cox
...
shapiro.test(x)
# very small p-value (reject the null hypothesis of normality)
# -> not a normal distribution indeed
shapiro.test(bc.x)
# p-value = 0.6814 (do not reject the null hypothesis of normality)

# RMK: these are non-linear transformations, so the normality of the
# transformed data does not imply the normality of the original data
# and that if I want to make inference on the mean of the original data
# and then transform using this non-linear transformation, what I get
# in terms of mean is not a transformation of the original mean
# (since the transformation is non-linear)
...
par(mfrow = c(2, 2))
hist(x, prob = TRUE, col = "grey85")
lines(0:1000 / 100,
   dnorm(0:1000 / 100, mean(x), sd(x)),
   col = "blue",
   lty = 2
)

hist(y, prob = TRUE, col = "grey85")
lines(0:1000,
   dnorm(0:1000, mean(y), sd(y)),
   col = "blue",
   lty = 2
)
...
shapiro.test(x)
# p-value for x is large enough (do not reject the null hypothesis of normality)
shapiro.test(y)
# p-value for y is very very small (reject the null hypothesis of normality)

graphics.off()

mshapiro.test(t(b))
# and in fact the p-value for the multivariate distribution of x and y is very
# small (-> reject the null hypothesis of normality)
...
lambda
# lambda[1] close to 1 -> in fact x is already quite normal
...
plot(
   xx,
   box_cox(x = xx, lambda = lambda$lambda[1]),
   col = "red",
   lty = 1,
   type = "l",
   xlab = expression(x),
...
legend(
   "bottomright",
   c(
      expression(lambda[x]), expression(lambda[y]),
      expression(paste(lambda, "=1"))
   ),
   col = c("blue", "red", "grey"),
   lty = c(1, 1, 1)
...
# RMK: x was already quite normal, so we expect a high p-value
shapiro.test(x)$p
shapiro.test(BC.x)$p
# with the box-cox transformation it is even more normal
shapiro.test(hyp.x)$p

# RMK: y was not normal, so we expect an improvement with the
# box-cox transf. and the hyp.
shapiro.test(y)$p
shapiro.test(BC.y)$p
shapiro.test(hyp.y)$p
# p-value is almost 1, so we can assume that the transformed data are Gaussian!

# MC-Shapiro test (H0: multivariate gaussianity)
mshapiro.test(rbind(x, y))$p
# very small p-value, so we reject H0 of the original data
mshapiro.test(rbind(BC.x, BC.y))$p
# very high p-value, so we accept H0 of the box-cox transformed data
mshapiro.test(rbind(hyp.x, hyp.y))$p
# almost 1 p-value, so we accept H0 of our hypothesis of transformed data
...
# Remove outliers
stiff_mean <- colMeans(stiff)
stiff_mean
stiff_cov <- cov(stiff)

d_mahalanobis <- matrix(mahalanobis(stiff, stiff_mean, stiff_cov))
...
lines(
   xx,
   dnorm(
      xx, mean(stiff_wo_outliers[, 1]),
      sd(stiff_wo_outliers[, 1])
   ),
   col = "blue",
   lty = 2
)
yy <- seq(1150, 2800, length = 100)
hist(stiff_wo_outliers[, 2],
   prob = TRUE,
   breaks = 8,
   col = "grey85"
)
lines(
   yy,
   dnorm(
      yy, mean(stiff_wo_outliers[, 2]),
      sd(stiff_wo_outliers[, 2])
   ),
   col = "blue",
   lty = 2
)
zz <- seq(1000, 2420, length = 100)
hist(stiff_wo_outliers[, 3],
   prob = TRUE,
   breaks = 8,
   col = "grey85"
)
lines(
   zz,
   dnorm(
      zz, mean(stiff_wo_outliers[, 3]),
      sd(stiff_wo_outliers[, 3])
   ),
   col = "blue",
   lty = 2
)
ww <- seq(1100, 2600, length = 100)
hist(stiff_wo_outliers[, 4],
   prob = TRUE,
   breaks = 8,
   col = "grey85"
)
lines(
   ww,
   dnorm(
      ww, mean(stiff_wo_outliers[, 4]),
      sd(stiff_wo_outliers[, 4])
   ),
   col = "blue",
   lty = 2
)

qqnorm(stiff_wo_outliers[, 1], main = "QQplot of V1")
qqline(stiff_wo_outliers[, 1])

qqnorm(stiff_wo_outliers[, 2], main = "QQplot of V2")
qqline(stiff_wo_outliers[, 2])

qqnorm(stiff_wo_outliers[, 3], main = "QQplot of V3")
qqline(stiff_wo_outliers[, 3])

qqnorm(stiff_wo_outliers[, 4], main = "QQplot of V4")
qqline(stiff_wo_outliers[, 4])

shapiro.test(stiff_wo_outliers[, 1])
shapiro.test(stiff_wo_outliers[, 2])
shapiro.test(stiff_wo_outliers[, 3])
shapiro.test(stiff_wo_outliers[, 4])
...
# Plot of the transformed variables with outliers removed

lambda.wo.outliers <- powerTransform(stiff_wo_outliers)
lambda.wo.outliers

BC.woutliers.x <- bcPower(stiff_wo_outliers[, 1], lambda.wo.outliers$lambda[1])
BC.woutliers.y <- bcPower(stiff_wo_outliers[, 2], lambda.wo.outliers$lambda[2])
BC.woutliers.z <- bcPower(stiff_wo_outliers[, 3], lambda.wo.outliers$lambda[3])
BC.woutliers.w <- bcPower(stiff_wo_outliers[, 4], lambda.wo.outliers$lambda[4])

mean(BC.woutliers.x)

par(mfrow = c(2, 4))
xx <- seq(70, 125, length = 100)
hist(BC.woutliers.x,
   prob = TRUE,
   breaks = 8,
   col = "grey85"
...
lines(
   xx,
   dnorm(
      xx, mean(BC.woutliers.x),
      sd(BC.woutliers.x)
   ),
   col = "blue",
   lty = 2
)
yy <- seq(11, 15, length = 100)
hist(BC.woutliers.y,
   prob = TRUE,
   breaks = 8,
   col = "grey85"
...
lines(
   yy,
   dnorm(
      yy, mean(BC.woutliers.y),
      sd(BC.woutliers.y)
   ),
   col = "blue",
   lty = 2
)
zz <- seq(10, 15, length = 100)
hist(BC.woutliers.z,
   prob = TRUE,
   breaks = 8,
   col = "grey85"
...
lines(
   zz,
   dnorm(
      zz, mean(BC.woutliers.z),
      sd(BC.woutliers.z)
   ),
   col = "blue",
   lty = 2
)
ww <- seq(5, 10, length = 100)
hist(BC.woutliers.w,
   prob = TRUE,
   breaks = 8,
   col = "grey85"
...
qqnorm(BC.woutliers.x, main = "QQplot of BC.woutliers.x")
qqline(BC.woutliers.x)

qqnorm(BC.woutliers.y, main = "QQplot of BC.woutliers.y")
qqline(BC.woutliers.y)

qqnorm(BC.woutliers.z, main = "QQplot of BC.woutliers.z")
qqline(BC.woutliers.z)

qqnorm(BC.woutliers.w, main = "QQplot of BC.woutliers.w")
qqline(BC.woutliers.w)


shapiro.test(BC.woutliers.x)
shapiro.test(BC.woutliers.y)
shapiro.test(BC.woutliers.z)
shapiro.test(BC.woutliers.w)

mshapiro.test(
   rbind(BC.woutliers.x, BC.woutliers.y, BC.woutliers.z, BC.woutliers.w)
)
# still not Gaussian
...
# RMK: Box-Cox transformations are useful only for univariate data.
# For multivariate data, we need to use multivariate transformations.
# There is a Box-plot transformation for multivariate data, but it is not
# worth it.
...
mshapiro.test(t(x))
# Distribution seems Gaussian

alpha <- 0.01
mu0 <- c(1, 0)

# T2 Statistics
# (it's basically the squared Mahalanobis distance of the sample mean from mu0)
x.T2 <- n * (x.mean - mu0) %*% x.invcov %*% (x.mean - mu0)
x.T2
# Radius of the ellipsoid
# this is Hotelling's T2 statistics
# https://en.wikipedia.org/wiki/Hotelling's_T-squared_distribution#Hotelling_t-squared_statistic
cfr.fisher <- ((n - 1) * p / (n - p)) * qf(1 - alpha, p, n - p)
# Test:
x.T2 < cfr.fisher
# test statistic is smaller than the critical value
# -> no statistical evidence to reject H0 at level alpha
# Rejection region is: {x.T2 > cfr.fisher}
...
# => we cannot reject for any reasonable level
# (we would reject for a level alpha > 81.7%)

dev.off()

# Region of rejection (centered in mu0)
# (not rejection region is inside the ellipse)
plot(x, asp = 1)
ellipse(
   mu0,
   shape = x.cov / n,
...
# (the sample mean is inside the ellipse, and hence we cannot reject H0)
points(x.mean[1],
   x.mean[2],
   pch = 16,
   col = "red",
...
# The probability that the true mean is contained in the confidence region
# is 99%


# RMK: the radius and the shape of the ellipse are the same,
...
# (we are going to use an asymptotic test, and this works for any distribution,
# as long as the sample size is large enough (Central Limit Theorem))
...
# If my sample mean is inside the exact confidence region (i.e., the one
# computed with the Fisher distribution) then I cannot reject H0, but I can
# reject H0 if I use the asymptotic confidence region (i.e., the one
# computed with the chi-square distribution) since it may be outside the
...
x.T2 < cfr.fisher
# I reject H0 at level alpha (test statistic is bigger than the critical value)

# Compute the p-value
P <- 1 - pf(x.T2 * (n - p) / ((n - 1) * p), p, n - p)
P
# and in fact the p-value is small
...
# the sample mean is inside the rejection region, so we reject H0
