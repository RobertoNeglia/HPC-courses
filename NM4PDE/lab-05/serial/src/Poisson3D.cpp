...
Poisson3D::setup() {
  ...
  // Initialize the finite element space.
  {
    ...
    std::cout << "  Quadrature points per boundary cell = " << quadrature_boundary->size()
              << std::endl;
  }
...
Poisson3D::solve() {
  ...
  // GMRES method for non symmetric matrices or preconditioners
  ...
  // SOR preconditioner
  // if the scalar r is 1, then this is GaussSeidel preconditioner
  // this is not symmetric, so I can't use the CG as a solver
  // PreconditionSOR<SparseMatrix<double>> preconditioner;
  // preconditioner.initialize(system_matrix,
  //                           PreconditionSOR<SparseMatrix<double>>::AdditionalData(1.0));

  // SSOR preconditioner
  // SOR generalization, that is symmetric
