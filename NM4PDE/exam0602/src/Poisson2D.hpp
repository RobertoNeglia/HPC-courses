...
public:
  ...
  public:
    ...
    virtual double
    value(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      return M_PI * M_PI * std::sin(M_PI * p[1]) * std::exp(p[0] + p[1]);
    }
  };
  ...
  public:
    ...
    virtual double
    value(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      if (p[0] == 0 && p[1] > 0 && p[1] < 1) // gamma 0
        return -std::sin(M_PI * p[1]) * std::exp(p[1]);
      if (p[0] == 1 && p[1] > 0 && p[1] < 1) // gamma 1
  ...
  public:
    ...
    virtual double
    value(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      return std::sin(M_PI * p[1]) * std::exp(p[0] + p[1]);
    }
    ...
    gradient(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      ...
      result[0] = std::sin(M_PI * p[1]) * std::exp(p[0] + p[1]);
      result[1] = std::sin(M_PI * p[1]) * std::exp(p[0] + p[1]) +
                  M_PI * std::cos(M_PI * p[1]) * std::exp(p[0] + p[1]);
