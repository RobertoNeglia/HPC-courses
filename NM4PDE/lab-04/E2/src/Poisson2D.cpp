...
Poisson2D::setup() {
  ...
  // Create the mesh.
  {
    ...
    std::ifstream mesh_in_file("../mesh/mesh-u-" + std::to_string(N + 1) + ".msh");
      ...
      cell_rhs    = 0.0;
        ...
        for (unsigned int q = 0; q < n_q; ++q) {
            ...
            for (unsigned int i = 0; i < dofs_per_cell; ++i) {
              ...
              cell_rhs(i) += forcing_term.value(fe_values.quadrature_point(q)) // f(x)
                           * fe_values.shape_value(i, q)                       // phi_i(x)
                           * fe_values.JxW(q);                                 // dx
            }
        }

      // Neumann bcs are not needed in this case, since h = 0.0
