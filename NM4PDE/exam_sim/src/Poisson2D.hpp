...
public:
  ...
  class FunctionU_R : public Function<dim> {
  public:
    // Constructor.
    FunctionU_R() {}
  ...
  public:
    ...
    gradient(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      ...
      result[0] = M_PI * p[1] * std::cos(M_PI * p[0] * p[1]);
      result[1] = M_PI * p[0] * std::cos(M_PI * p[0] * p[1]);
...
protected:
  ...
  // Robin boundary condition coefficient.
  FunctionLambda function_lambda;

  // Robin boundary condition constant.
  FunctionU_R function_u_r;
