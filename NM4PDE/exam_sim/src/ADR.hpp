...
public:
  ...
  class FunctionC : public Function<dim> {
  public:
    // Constructor.
    FunctionC() {}
  ...
  class FunctionR : public Function<dim> {
  public:
    // Constructor.
    FunctionR() {}
  ...
  public:
    ...
    virtual double
    value(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      return function_mu.value(p) * 2.0 * M_PI * M_PI * std::sin(M_PI * p[0]) *
               std::sin(M_PI * p[1]) // Diffusion term
           + function_b.value(p, 0) * M_PI * std::cos(M_PI * p[0]) *
               std::sin(M_PI * p[1]) +
             function_b.value(p, 1) * M_PI * std::sin(M_PI * p[0]) *
               std::cos(M_PI * p[1]) // Advection term
           + function_c.value(p) * std::sin(M_PI * p[0]) *
               std::sin(M_PI * p[1]); // Reaction term
    }

  protected:
    FunctionMu function_mu;
    FunctionB  function_b;
    FunctionC  function_c;
  };
  ...
  public:
    ...
    gradient(const Point<dim> &p, const unsigned int /*component*/ = 0) const {
      ...
      result[0] = M_PI * std::cos(M_PI * p[0]) * std::sin(M_PI * p[1]);
      result[1] = M_PI * std::sin(M_PI * p[0]) * std::cos(M_PI * p[1]);
