    ...
    for (const auto &cell : dof_handler.active_cell_iterators()) {
      ...
      cell_rhs    = 0.0;
        ...
        for (unsigned int q = 0; q < n_q; ++q) {
          ...
          Vector<double> b(dim);
          function_b.vector_value(fe_values.quadrature_point(q), b);
          Tensor<1, dim> b_tensor;
            for (unsigned int d = 0; d < dim; d++) {
              b_tensor[d] = b[d];
            }
          // pcout << "POST VECTOR VALUE" << std::endl;
                ...
                for (unsigned int j = 0; j < dofs_per_cell; ++j) {
                  ...
                  cell_matrix(i, j) +=
                    function_mu.value(fe_values.quadrature_point(q)) // mu(x)
                      ...
                      * fe_values.JxW(q)                             // dx
                    + scalar_product(b_tensor, fe_values.shape_grad(i, q)) *
                        fe_values.shape_value(j, q) * fe_values.JxW(q) +
                    function_c.value(fe_values.quadrature_point(q)) *
                ...
                if (cell->face(face_number)->at_boundary() &&
                  ...
                  fe_values_boundary.reinit(cell, face_number);
                    ...
                    for (unsigned int q = 0; q < quadrature_boundary->size();
                        ...
                        for (unsigned int i = 0; i < dofs_per_cell; i++) {
                          cell_rhs(i) += function_r.value(
                                           fe_values_boundary.quadrature_point(q)) // r(x)
