...
public:
  ...
  virtual Vector
  vmult(const Vector &v) const = 0;
  // element access in read only (const version)
  virtual const elem_t &
  operator()(size_t i, size_t j) const = 0;
  // element access in write (returns non-const reference)
  virtual elem_t &
  operator()(size_t i, size_t j) = 0;
  // virtual destructor
...
protected: // protected because need to be accessible to children!
  ...
  size_t nnz;
  virtual void
  _print(std::ostream &os) const = 0;
};

class MapMatrix : public SparseMatrix {
public:
  virtual Vector
  vmult(const Vector &v) const override {
    ...
    // for every row i...
      ...
      for (size_t i = 0; i < data.size(); i++) {
          ...
          for (const auto [key, value] : data[i]) {
            ...
            // !!RMK¡¡: only elements that are present in the matrix are considered, others are zero
            // (not how the key value is used also as input vector index)
  ...
  // element access in read only (const version)
  virtual const elem_t &
  operator()(size_t i, size_t j) const override {
    return data.at(i).at(j);
  }

  // element access in write (returns non-const reference)
  virtual elem_t &
  operator()(size_t i, size_t j) override {
      // Row boundary check
      if (i + 1 > data.size()) { // must use this notation and not i > data.size() - 1 because it
                                 // returns a size_t, that cant
        // be negative
    ...
    const auto it = data[i].find(j);
      if (it == data[i].end()) // if the element in (i, j) is not present
      {
        ...
        // emplace returns pair of an iterator to the newly created pair in the map, and a bool that
        // denotes if the operation was successfull: so i take the pair pointed by the iterator (so
        // (*data|...|.first) and the return the reference to the mapped value through the key)
  ...
  void
  printDataSize(std::ostream &os = std::cout) {
...
protected:
  virtual void
  _print(std::ostream &os) const override {
      for (size_t i = 0; i < data.size(); i++) {
          for (const auto [key, value] : data[i]) {
...
void
fill_matrix(SparseMatrix &m, size_t n) {
...
main() {
...

  {
    ...
    std::cout << "Time elapsed to compute matrix-vector multiplication with ordered matrix: "
              << diff << std::endl;
  }

  {
    ...
    std::cout << "Time elapsed to compute matrix-vector multiplication with unordered matrix: "
              << diff << std::endl;
  }
